# 第一章 CLI + E2E 示例

这一节在“满 100 元免运费”的函数测试基础上，再加一层**用户交互入口**，让你看到什么叫端到端测试。

---

## 为什么需要 CLI

纯函数的测试已经能验证核心逻辑，但现实项目里，用户往往不是直接调用函数，而是通过：

- 命令行
- Web 表单
- 接口请求
- 后台任务

CLI 是最简单的用户入口，因此很适合拿来演示 E2E。

---

## 本示例做了什么

用户输入：

```text
100
```

程序输出：

```text
请输入订单金额：订单满 100 元，免运费
```

如果输入：

```text
-1
```

程序输出：

```text
订单金额不能为负数
```

---

## 代码位置

- CLI 入口：`src/tdd_learning_python/chapter1_cli.py`
- 模块入口：`src/tdd_learning_python/__main__.py`
- E2E 测试：`tests/test_cli_e2e.py`

---

## 如何运行

先确保依赖已经同步：

```bash
uv sync --extra dev
```

然后按下面两种方式之一运行：

### 方式 1：推荐，使用 uv

```bash
uv run python -m tdd_learning_python.chapter1_cli
```


### 方式 2：直接使用虚拟环境里的 Python

```bash
./.venv/bin/python -m tdd_learning_python.chapter1_cli
```


然后输入一个金额，例如：

```text
100
```

---

## 如何测试

端到端测试会真正启动一个独立进程，向它输入内容，再检查输出结果：

```bash
./.venv/bin/pytest tests/test_cli_e2e.py -q
```

---

## 你应该观察什么

- 输入 `100` 时，程序返回免运费提示
- 输入 `-1` 时，程序返回错误提示并以非 0 退出
- 这个测试覆盖的是“用户怎么用这个程序”，不是内部函数实现细节

这就是 E2E 的价值：它能帮你确认**从入口到结果**这条链路是通的。
